...
a_2d_nan = np.array([[1, 1, 1, 3, 3, 3, 5, 5],
                     [1, np.nan, 1, 3, 3, 3, 5, 5],
                     [1, 1, 1, 3, 3, 3, 5, 5],
                     [2, 2, 2, 4, 4, 4, 6, 6],
                     [2, 2, 2, 4, 4, 4, 6, 6],
                     [2, 2, 2, 4, 4, 4, 6, 6]], dtype=np.float)
...
grid_dxC_ll, grid_dyC_ll = dll_dist(grid_dxC, grid_dyC,
                                    grid_XC, grid_YC,
                                    xarray=False)

grid_dxG_ll, grid_dyG_ll = dll_dist(grid_dxG, grid_dyG,
                                    grid_XG, grid_YG,
                                    xarray=False)
...
dataarrays = {
    ...
    'dataarray_2d_example':
        xr.DataArray(da.from_array(a_2d_nan, a_2d_nan.shape, name='a_2d_nan'),
                     coords={'j': (['j', ], np.arange(0, 6)),
                             'i': (['i', ], np.arange(0, 8))},
                     dims=['j', 'i']
                     ),
    'dataarray_2d_ones':
        xr.DataArray(da.from_array(ones_2d, ones_2d.shape, name='ones_2d'),
                     coords={'j': (['j', ], np.arange(0, 6)),
                             'i': (['i', ], np.arange(0, 8))},
                     dims=['j', 'i']
                     ),
    'dataarray_2d_ones_nan':
        xr.DataArray(da.from_array(ones_2d_nan, ones_2d_nan.shape,
                                   name='ones_2d_nan'),
                     coords={'j': (['j', ], np.arange(0, 6)),
                             'i': (['i', ], np.arange(0, 8))},
                     dims=['j', 'i'])

...
from xarrayutils.weighted_operations import (
    _broadcast_weights,
    weighted_mean,
    weighted_sum,
    weighted_sum_raw,
...
def test_broadcast_weights():
    ...
    weight_expanded_test = xr.DataArray(
        np.array([[np.nan, 3, 5], [1, 3, 5]]), dims=["x", "y"], name="data", attrs=attrs
    )
    weight_dsa_expanded_test = xr.DataArray(
        dsa.from_array(weight_expanded_test, chunks=chunks),
        dims=["x", "y"],
        name="data",
        attrs=attrs,
    )
    ...
    a_dsa = xr.DataArray(
        dsa.from_array(data_test, chunks=chunks),
        dims=["x", "y"],
        name="data",
        attrs=attrs,
    )
    w_dsa = xr.DataArray(
        dsa.from_array(weight_test, chunks=chunks), dims=["y"], name="data", attrs=attrs
    )
    ...
    xr.testing.assert_identical(w_dsa_expanded_attrs, weight_dsa_expanded_test)
...
def test_weighted_sum_raw():
    ...
    w_dsa = xr.DataArray(
        dsa.from_array(weight_test, chunks=chunks), dims=["y"], attrs=attrs
    )

    data_sum, weight_sum = weighted_sum_raw(
        a_dsa, w_dsa, dim=["x", "y"], dim_check=False
    )
    data_sum_attrs, weight_sum_attrs = weighted_sum_raw(
        a_dsa, w_dsa, dim=["x", "y"], dim_check=False, keep_attrs=True
    )
...
def test_weighted_mean():
    ...
    mean = weighted_mean(a_dsa, w_dsa, dim=["x", "y"], dim_check=False)
    ymean = weighted_mean(a_dsa, w_dsa, dim=["y"], dim_check=False)
    xmean = weighted_mean(a_dsa, w_dsa, dim=["x"], dim_check=False)
    xmean_alt = weighted_mean(a_dsa, w_dsa, dim="x", dim_check=False, keep_attrs=True)

    with pytest.raises(RuntimeError):
        weighted_mean(a_dsa, w_dsa, dim=["x", "y"], dim_check=True)
...
def test_weighted_sum():
    ...
    mean = weighted_sum(a_dsa, w_dsa, dim=["x", "y"], dim_check=False)

    ymean = weighted_sum(a_dsa, w_dsa, dim=["y"], dim_check=False)
    xmean = weighted_sum(a_dsa, w_dsa, dim=["x"], dim_check=False)
    xmean_alt = weighted_sum(a_dsa, w_dsa, dim="x", dim_check=False, keep_attrs=True)

    with pytest.raises(RuntimeError):
        weighted_sum(a_dsa, w_dsa, dim=["x", "y"], dim_check=True)
